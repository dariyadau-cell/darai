...
## 3. Ключ AI-провайдера (для оценки и подсказок)

Нужен один ключ, **только в переменных окружения Vercel** (шаг 5) — он никогда не попадает в код или в браузер:

- **`DEEPSEEK_API_KEY`** — используется и для оценки эссе/speaking (`/api/grade`), и для коротких AI-подсказок к черновику (`/api/assist`), модель DeepSeek. Завести на [platform.deepseek.com](https://platform.deepseek.com) → API Keys.

Платный по факту использования. Если вставляли ключ прямо в чат/переписку где-либо — стоит перевыпустить его в кабинете провайдера и вписать уже новый.
